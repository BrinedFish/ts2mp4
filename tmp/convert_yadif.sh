...
TMP_IM_MP4_NAME=$(echo "$TS_NAME" | sed -e s/\.ts$/.im.mp4/)
PROGRAM_ID=340
...
/home/jnakano/ffmpeg/bin/ffmpeg -y -i "$TS_NAME" $PROGRAM_OPT_V -c:v libx264 -preset ultrafast -qp 0 -filter:v yadif=1 -s 720x480 $PROGRAM_OPT_A -c:a libfdk_aac "$TMP_IM_MP4_NAME"
